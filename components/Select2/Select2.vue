    ...
    >
      ...
      >
        ...
        <template v-if="mode !== 'tags' && searchable && !disabled">
          <input :type="inputType" :modelValue="search" :value="search" :class="classList.search"
            :autocomplete="autocomplete" :id="searchable ? id : undefined" @input="handleSearchInput"
            @keypress="handleKeypress" @paste.stop="handlePaste" ref="input" :aria-controls="ariaControls"
            :aria-placeholder="ariaPlaceholder" :aria-expanded="isOpen" :aria-activedescendant="ariaActiveDescendant"
            :aria-multiselectable="ariaMultiselectable" role="combobox" v-bind="{
              ...attrs,
              ...arias,
            }" />
        </template>
        ...
        <template v-if="mode == 'tags'">
          <div :class="classList.tags" style="width: calc(100% - 2rem);" data-tags>

            <slot v-for="(option, i, key) in activeListed.items" name="tag" :option="option"
              :handleTagRemove="handleTagRemove" :disabled="disabled">
              <span :class="classList.tag" tabindex="-1" @keyup.enter="handleTagRemove(option, $event)" :key="key"
                :aria-label="ariaTagLabel(option[label])">
                <p class="truncate">{{ option[label] }}</p>
                <y-button size="2xs" circle v-if="!disabled" :class="classList.tagRemove"
                  @click.stop="handleTagRemove(option, $event)">
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M4.66699 4.66602L11.3337 11.3327" stroke="#9AA1B4" stroke-width="1.5" stroke-linecap="round"
                      stroke-linejoin="round" />
                    <path d="M4.66699 11.3327L11.3337 4.66602" stroke="#9AA1B4" stroke-width="1.5" stroke-linecap="round"
                      stroke-linejoin="round" />
                  </svg>
            ...
            <div :class="classList.tagsSearchWrapper" ref="tags">
              ...
              <!-- Actual search input -->
              <input v-if="searchable && !disabled" :type="inputType" :modelValue="search" :value="search"
                :class="classList.tagsSearch" :id="searchable ? id : undefined" :autocomplete="autocomplete"
                @input="handleSearchInput" @keypress="handleKeypress" @paste.stop="handlePaste" ref="input"
                :aria-controls="ariaControls" :aria-placeholder="ariaPlaceholder" :aria-expanded="isOpen"
                :aria-activedescendant="ariaActiveDescendant" :aria-multiselectable="ariaMultiselectable" role="combobox"
                v-bind="{
                  ...attrs,
                  ...arias,
                }" />
        ...
        <template v-if="mode == 'multiple' && hasSelected && !search">
          <div class="z-1">
            ...
            <slot name="selected-item" v-for="(option, i) in activeListed.items"
              :key="i"
              :option="option"
            ...
            >
              ...
              <template v-if="(i + 1) < activeListed.items.length">
                ,
              </template>
            </slot>
            <slot name="more" v-if="mode === 'multiple' && activeListed && activeListed.more > 0"
        ...
        <slot v-if="loading || resolving" name="spinner">
          <span :class="classList.spinner" aria-hidden="true">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
              ...
              <path :fill="spinnerBg" fill-rule="evenodd" clip-rule="evenodd"
              ...
              <path :fill="spinnerColor" fill-rule="evenodd" clip-rule="evenodd"
        ...
        <slot v-if="hasSelected && !disabled && canClear && !busy" name="clear" :clear="clear">
          <span aria-hidden="true" tabindex="0" role="button" data-clear aria-roledescription="❎" :class="classList.clear"
            @click="clear" @keyup.enter="clear"><span :class="classList.clearIcon"></span></span>
        </slot>

        <!-- Caret -->
        <slot v-if="caret && showOptions" name="caret">
          <span :class="classList.caret" @click.stop="handleCaretClick" aria-hidden="true">
        ...
        <div ref="optionContainer" :class="classList.dropdown" tabindex="-1">
          ...
          <ul :class="classList.options" :id="ariaControls" role="listbox">
            ...
            <template v-if="groups">
              <li v-for="(group, i, key) in fg" :class="classList.group" :key="key" :id="ariaGroupId(group)"
                :aria-label="ariaGroupLabel(group)" :aria-selected="isSelected(group)" role="option">
                <div :class="classList.groupLabel(group)" :data-pointed="isPointed(group)"
                  @mouseenter="setPointer(group, i)" @click="handleGroupClick(group)">
                  <slot name="grouplabel" :group="group" :is-selected="isSelected" :is-pointed="isPointed">
                    <span v-html="group[groupLabel]"></span>
                  </slot>
                </div>

                <ul :class="classList.groupOptions" :aria-label="ariaGroupLabel(group)" role="group">
                  <li v-for="(option, i, key) in group.__VISIBLE__" :class="classList.option(option, group)"
                    :data-pointed="isPointed(option)" :data-selected="isSelected(option) || undefined" :key="key"
                    @mouseenter="setPointer(option)" @click="handleOptionClick(option)" :id="ariaOptionId(option)"
                    :aria-selected="isSelected(option)" :aria-label="ariaOptionLabel(option)" role="option">
                    <slot name="option" :option="option" :is-selected="isSelected" :is-pointed="isPointed"
                      :search="search">
            ...
            <template v-else>
              <li v-for="(option, i, key) in fo" :class="classList.option(option)" :data-pointed="isPointed(option)"
                :data-selected="isSelected(option) || undefined" :key="key" @mouseenter="setPointer(option)"
                @click.stop="handleOptionClick(option)" :id="ariaOptionId(option)" :aria-selected="isSelected(option)"
                :aria-label="ariaOptionLabel(option)" role="option">
